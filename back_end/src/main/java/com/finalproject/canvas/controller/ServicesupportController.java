...
public class ServicesupportController {
    ...
    @GetMapping("/list/all")
    public ResponseEntity<List<ServicesupportEntity>> getAnswer(
            @RequestParam(value = "writer", required = false) String writer) {
        List<ServicesupportEntity> list = writer != null
                ? servicesupportService.mypage(writer)
                : servicesupportService.getAllSupport(); // 전체 조회 메서드 추가
        return ResponseEntity.ok(list);
    }
}